...
const Messages: FC<MessagesProps> = ({ chatId, initialMessages, sessionId, sessionImg, chatPartner }) => {
  ...
  useEffect(() => {
    ...
    const messageRequestHandler = (message: Message) => {
      setMessages((prev) => [message, ...prev])
    }
    
    pusherClient.bind('incoming-message', messageRequestHandler)
    
    return () => {
      pusherClient.unsubscribe(toPusherKey(`chat:${chatId}`));
      pusherClient.unbind('incoming-message', messageRequestHandler)
    }
  }, [chatId])
